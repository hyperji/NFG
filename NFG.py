...
print("NFG reforged, activation amount backed")
...
class NFG4img_v2(tf.keras.layers.Layer):
    def __init__(self, ksize, strides, d_neuron, dk, dv, Nh, dact, final_dim, padding='SAME'):
        ...
        self.wkq = tf.keras.layers.Conv2D(filters=dk + dk, kernel_size=1, strides=1, activation=None)


        self.wv = tf.keras.layers.Conv2D(filters=dv, kernel_size=1, strides=1)

        self.wgactq = tf.keras.layers.Conv2D(filters=dact, kernel_size=1, strides=1, activation=None)
        self.wgactk = tf.keras.layers.Conv2D(filters=dact, kernel_size=1, strides=1, activation=None)
        ...
        self.wf = tf.keras.layers.Conv2D(filters=final_dim, kernel_size=1, strides=1)
    ...
    def compute_neural_structures(self):
        H, W, _ = self.neuron_embeddings.shape
        kq = self.wkq(tf.expand_dims(self.neuron_embeddings, axis=0))
    ...
    def compute_activation_amounts(self, inps):
        ...
        actk = self.wgactk(inps)
        actq = self.wgactq(tf.expand_dims(self.neuron_embeddings, axis=0))
        ...
        flat_actk = tf.reshape(actk, [tf.shape(inps)[0], self.Nh, vH*vW, self.dacth]) #[128, 10, 196, 8]
        flat_actq = tf.reshape(actq, [1, self.Nh, H*W, self.dacth])
...
class NFG4graph(tf.keras.layers.Layer):
    def __init__(self, ksize, strides, d_neuron, dk, dv, Nh, dact, final_dim, padding='SAME'):
        """
        :param ksize: 窗口大小
        :param strides: 步长大小
    ...
    def compute_activation_amounts(self, inps):
        ...
        sum_glogits = tf.reduce_sum(glogits, axis=-1, keepdims=True)
        #print("sum_glogits", sum_glogits.shape)
        activations = (vH*vW / (sum_glogits + 1e-6)) * glogits
        #print("gweights with norm", gweights.shape)
